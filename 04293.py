            ...
            if gdf_geojson.empty:
                ...
                if merged_data.empty:
                    ...
                    for _, row in gdf_merged.iterrows():
                        ...
                        # 判斷經度是否相近，調整文字位置
                        if abs(station_coords[0] - minx) < 0.005:  # 經度相近的點
                            ax.text(
                                station_coords[0],  # 維持經度不變
                                station_coords[1] + 0.001,  # 緯度上移
                                arrival_status,  # 顯示到達時間
                                fontsize=6,  # 調整字體大小為更小
                                color="blue"
                            )
                        else:
                            # 在點的右邊顯示到達時間
                            ax.text(
                                station_coords[0] + 0.0015,  # 經度右移
                                station_coords[1],  # 維持緯度不變
                                arrival_status,  # 顯示到達時間
                                fontsize=6,  # 調整字體大小為更小
                                color="blue"
                            )
