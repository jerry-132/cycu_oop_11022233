    ...
    with sync_playwright() as p:
        ...
        for stop in stop_elements:
            try:
                ...
                # 將資料加入列表
                stop_details.append((stop_name, latitude, longitude))
...
def save_to_csv(data, filename):
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        # 寫入標題
        writer.writerow(["公車代碼", "站點名稱", "緯度", "經度"])
        # 寫入資料
        for route_id, stops in data.items():
            for stop_name, latitude, longitude in stops:
                writer.writerow([route_id, stop_name, latitude, longitude])
...
if __name__ == "__main__":
    try:
        ...
        for route_id in route_ids:
            stop_details = fetch_bus_stop_details(route_id)
            all_stops[route_id] = stop_details

        # 儲存為 CSV
        output_file = "taipei_bus_stop_details.csv"
